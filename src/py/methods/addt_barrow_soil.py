...
def addt_method():
    ...
    def zero_isotherm_depth(row):
        ...
        for i in range(len(depths) - 1):
            ...
            if row[depths[i]] > freezing_point and row[depths[i+1]] < freezing_point:
                ...
                slope = (row[depths[i+1]] - row[depths[i]]) / (depths[i+1] - depths[i])
                zero_crossing_depth = depths[i] + (freezing_point - row[depths[i]]) / slope
    ...
    df_soil_temp = pd.merge(df_soil_temp, df_air_temp, on=['year', 'month', 'day'], how='left')
    del df_air_temp
