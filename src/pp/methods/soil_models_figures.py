...
ax1.set_xlabel("Thaw Depth $h$ (meters)")
ax1.set_ylabel("Porosity $P$")
...
ax2.set_xlabel("Thaw Depth $h$ (meters)")
ax2.set_ylabel(r"Subsidence $\delta \, (m)$")
...
# Third plot
sqrt_ddt_ratio = 3.5
plot_sqrt_ddt(liu_smm, 'b', sqrt_ddt_ratio, ax3, ylim=(0.0, 0.025))
plot_sqrt_ddt(const_smm, 'r', sqrt_ddt_ratio, ax3, linestyle='dashed')

# Fourth plot
sqrt_ddt_ratio = 1.5
plot_sqrt_ddt(liu_smm, 'b', sqrt_ddt_ratio, ax4, ylim=(0.0, 0.025))
...
plot_sqrt_ddt(inv_smm, color, 3.5, ax3, ylim=(0.0, 0.004))
plot_sqrt_ddt(inv_smm, color, 1.5, ax4, ylim=(0.0, 0.004))
...
def plot_ddt(smm: SoilMoistureModel, sdi: SoilDepthIntegration, ddt_ratio, ax, linestyle=None, color=None, label=None):
    ...
    ax.set_title(fr"Subsidence Difference vs Thaw Depth for ADDT ratio $ = {ddt_ratio}$")
