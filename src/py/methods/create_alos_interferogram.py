...
def main(alos1, alos2):
    ...
    alos1_xml = f"alos_{alos1}.xml"
    alos2_xml = f"alos_{alos2}.xml"
    stripmapApp_xml = f"stripmapApp_{alos1}_{alos2}.xml"
    ...
    modify_stripmap_xml(STRIPMAP_APP_TEMPLATE_XML, stripmapApp_xml, alos1_xml, alos2_xml)

    print(f"Results will be saved to: {output_dir}")
    output_dir.mkdir(exist_ok=True, parents=True)

    insar = Insar(name="stripmapApp", cmdline=stripmapApp_xml)
    insar.configure()
    insar.run()
...
def modify_stripmap_xml(filename_in, filename_out, new_reference_value, new_secondary_value):
